...
var c=function(a){},d=function(c){var d=c||b.event;if(!(d.metaKey||d.ctrlKey||d.altKey||d.shiftKey)){for(var e=d.target;;){if("BODY"===e.tagName)return;if("A"===e.tagName||"AREA"===e.tagName)break;e=e.parentElement}var f=e;if(f.href){if(f.href===b.location.href)return console.log("you're already on that page"),void d.preventDefault();var g=new RegExp(b.location.origin+b.location.pathname+"[^/]*[&#?]","g"),h=new RegExp("/wp-","g");
if((f.href.indexOf(a.domain)>-1||f.href.indexOf(":")===-1)&&!f.href.match(g)&&!f.href.match(h)&&!f.href.match(/\/feed/g)){d.preventDefault(),console.log(f);for(var i={postExpand:"post--postExpandJS"},j=f;;){if("BODY"===j.tagName)return;for(var k in i)if(j.classList.contains(i[k]))return b.history.pushState({type:k},"",f.href),void b[k](d,j,f.href);j=j.parentElement}}}}},e=function(){b.addEventListener("popstate",c,!1),
a.addEventListener("click",d,!0)};"loading"===a.readyState?a.addEventListener("DOMContentLoaded",e,!1):e()}(document,window),function(a,b){"use strict";b.postExpand=function(c,d,e){e=b.addAjaxQueryString(e,"page"),console.log(e);var f=new b.AnimateMutate({hero:"cover__hero",title:["cover__titleLink",!0],imgList:"cover__heroImg--list",imgBlur:"cover__heroImg--blur",loadBar:"button"},d),g=f.el.context,h=f.el.hero,i=f.el.title,j=f.el.imgList,k=f.el.loadBar,l=k.node.getElementsByClassName("button__loadbar")[0];
...
g.node.getElementsByClassName("frame")[0].innerHTML=b.getElementsByClassName("frame")[0].innerHTML};var o=function(){j.node.style.transition=f.el.imgBlur.node.style.transition="opacity .5s linear",b.requestAnimationFrame(function(){j.node.style.opacity=f.el.imgBlur.node.style.opacity=""})},p=function(){return n.readyState},q=[[p,4],[m.readyState,m.readyStateMax]],r=new b.Progress(q,o);r.update=function(){
l.style.transform="scaleX("+this.readyState+")"},n.send(),f.animate(),r.start(),m.mutateAtts()}}(document,window);