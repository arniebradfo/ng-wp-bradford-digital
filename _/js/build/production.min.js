...
void d.preventDefault();var g=new RegExp(b.location.origin+b.location.pathname+"[^/]*[&#?]","g"),h=new RegExp("/wp-","g");if((f.href.indexOf(a.domain)>-1||f.href.indexOf(":")===-1)&&!f.href.match(g)&&!f.href.match(h)&&!f.href.match(/\/feed/g)){d.preventDefault();for(var i={postExpand:"post--postExpandJS"},j=f;;){if("BODY"===j.tagName)return;for(var k in i)if(j.classList.contains(i[k]))return b.history.pushState({
...
m.setRequestHeader("WP-Request-Type","GetPage"),m.onload=function(){var b=a.createElement("div");b.innerHTML=m.responseText,console.log(b),g.node.getElementsByClassName("excerpt")[0].innerHTML="",g.node.getElementsByClassName("frame")[0].innerHTML=b.getElementsByClassName("frame")[0].innerHTML},m.send(),f.animate(),l.mutateAtts()}}(document,window);